...
from deep_compare import load_net
from Utils import L2Norm, draw_patches, filter_points
...
class L2net_des:
    def __init__(self, max_keypoints=200, patch_size=32,
                 model_src='./pretrained/l2net_L_N+.pt',
                 use_cuda=True):
...
class DeepCompareNet_des:
    def __init__(self, model, lua_model, patch_size=64, max_keypoints=200, use_cuda=True):
        self.use_cuda = use_cuda
        self.detector = cv2.xfeatures2d.SURF_create(350)
        self.net, self.params = load_net(model, lua_model)
        self.max_keypoints = max_keypoints
        self.ps = patch_size
    ...
    def compute(self, image, kp, bs=64, expansion_coef=1.3):
        ...
        patches = [cv2.resize(cv2.getRectSubPix(image,
                                                (round(k.size * expansion_coef),
                                                 round(k.size * expansion_coef)),
                                                k.pt),
                              (self.ps, self.ps)) for k in kp]
        patches = Variable(torch.FloatTensor(np.array(patches).reshape(-1, 1, self.ps, self.ps)))
        it = int(patches.size()[0] / bs)
        r = patches.size()[0] % bs
        ...
        for i in range(it):
            if self.use_cuda:
                des.append(self.net(patches[i*bs:(i+1)*bs].cuda(),
                                    self.params).cpu().data.numpy())
            else:
                des.append(self.net(patches[i*bs:(i+1)*bs],
                                    self.params).cpu().data.numpy())
        ...
        if r > 1:
            ...
            if self.use_cuda:
                rem = rem.cuda()
            des.append(self.net(rem,
                                self.params).cpu().data.numpy())

        return kp, np.concatenate(des, 0)

class HardNetBrown_des:
    def __init__(self, model_src='./pretrained/6Brown/hardnetBr6.pth',
...
class HardNetAll_des:
    def __init__(self, model_src='./pretrained/pretrained_all_datasets/HardNet++.pth',
...
class HardNetHPatches_des:
    def __init__(self, model_src='./pretrained/3rd_party/HardNetPS/HardNetPS.pth',
                 patch_size=32, max_keypoints=200, use_cuda=True):
        self.use_cuda = use_cuda
        self.detector = cv2.xfeatures2d.SURF_create(350)
        self.max_keypoints = max_keypoints
