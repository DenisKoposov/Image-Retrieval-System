...
from learned_descriptors import L2net_des, DeepCompareNet_des
from learned_descriptors import HardNetAll_des, HardNetBrown_des, HardNetHPatches_des
from learned_descriptors import SURF, SIFT
from matplotlib import pyplot as plt

from Utils import find_image_files, draw_result
from HKM import VocabularyTree
...
class Storage:
    ...
    def __init__(self, dir_path, training_path, des_type,
        ...
        if des_type == 'sift':
            ...
            self.fd = L2net_des(max_keypoints=max_keypoints, use_cuda=True)
        elif des_type == 'DeepCompare':
            self.fd = DeepCompareNet_des('siam2stream',
                                         './pretrained/deep_compare/siam2stream/siam2stream_liberty.t7',
                                         max_keypoints=max_keypoints, use_cuda=True)
        elif des_type == 'HardNetBrown':
            self.fd = HardNetBrown_des(max_keypoints=max_keypoints, use_cuda=True)
        elif des_type == 'HardNetAll':
            self.fd = HardNetAll_des(max_keypoints=max_keypoints, use_cuda=True)
        elif des_type == 'HardNetHPatches':
            self.fd = HardNetHPatches_des(max_keypoints=max_keypoints, use_cuda=True)
        else:
            self.fd = None

        self.des_path = 'des_{}.pkl'.format(self.des_type)
        self.des_path = os.path.join(self.dir, self.des_path)

        self.bow_path = 'bow_{}_{}.pkl'.format(self.des_type, label)
        self.bow_path = os.path.join(self.dir, self.bow_path)

        self.ca_path = 'clusterer_{}_{}.pkl'.format(self.des_type, label)
        self.ca_path = os.path.join(self.dir, self.ca_path)

        self.inverted_index_path = 'inverted_index_{}_{}.pkl'.format(self.des_type, label)
        self.inverted_index_path = os.path.join(self.dir, self.inverted_index_path)

        self.f_names_path = 'f_names_{}_{}.pkl'.format(self.des_type, label)
        self.f_names_path = os.path.join(self.dir, self.f_names_path)

        self.params_path = 'params_{}_{}.pkl'.format(self.des_type, label)
        self.params_path = os.path.join(self.dir, self.params_path)

        if (os.path.exists(self.inverted_index_path) and
            os.path.exists(self.f_names_path) and
            os.path.exists(self.params_path) and
            os.path.exists(self.ca_path) and
            os.path.exists(self.des_path) and
            os.path.exists(self.bow_path)):
            self.load()
            self.load_ca()
            if self.des_type != 'DeepCompare':
    ...
    def create_new_index(self, max_training_images=None):
        ...
        if os.path.abspath(self.dir) == os.path.abspath(self.training_path):
            ...
            print("Size in memory:", descriptors.shape[0] * descriptors.shape[1] * 4 / float(1024 ** 2))
        ...
        self.bow = np.zeros((len(self.f_names), self.n_words), dtype=np.float32)
        self.inverted_index = [set() for i in range(self.n_words)]

        freqs = np.zeros(self.n_words, dtype=np.int64)
        ...
        self.bow /= np.sum(self.bow, axis=1).reshape(-1, 1)
    ...
    def ransac(self, img_descriptor, kp, candidates,
        ...
        if self.des_type == "DeepCompare":
            descriptors = [self.get_descriptor(img[1], raw=True)
        ...
        for i, des in enumerate(descriptors):
            ...
            #matches = flann.knnMatch(img_descriptor,
            #                         des, k=2)
    ...
    def get_similar(self, img_path, n_candidates=100,
        ...
        img_descriptor, img_bovw, kp = self.get_descriptor(img_path, both=True)
        ...
        start = time.time()
        ranks = [(i, euclidean(img_bovw * self.idf,
                               self.bow[i] * self.idf)
                 for i in candidates]
        ...
        if qe_enable and new_query is None and np.count_nonzero(verified) < qe_limit:
            start = time.time()
            top_res = [self.bow[r[0][0]] for r in sv_candidates[:qe_avg]]
            ...
            new_sv_candidates = self.get_similar(img_path, n_candidates,
                                     ...
                                     qe_avg, qe_limit, one_more_query, debug,
        ...
        if debug and qe_enable and new_query == None:                                   
